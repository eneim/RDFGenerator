...
public class Generator {
	...
	/** class name strings */
	static final String[] CLASS_TOKEN = { "University", // CS_C_UNIV
	...
	static final String[] PROP_TOKEN = { "name", "takesCourse", "teacherOf",
	...
	private static final int UNDER_COURSE_NUM = 100; // must >= max faculty # *
														// FACULTY_COURSE_MAX
	/** size of the pool of the graduate courses for one department */
	private static final int GRAD_COURSE_NUM = 100; // must >= max faculty # *
	...
	/**
	 * Callback by the writer when it starts an instance section identified by
	...
	private void _setInstanceInfo() {
		...
		for (int i = 0; i < CLASS_NUM; i++) {
			...
			case CS_C_DEPT:
				break;
			case CS_C_FULLPROF:
				instances_[i].num = _getRandomFromRange(FULLPROF_MIN,
						FULLPROF_MAX);
				break;
			case CS_C_ASSOPROF:
				instances_[i].num = _getRandomFromRange(ASSOPROF_MIN,
						ASSOPROF_MAX);
				break;
			case CS_C_ASSTPROF:
				instances_[i].num = _getRandomFromRange(ASSTPROF_MIN,
						ASSTPROF_MAX);
				break;
			case CS_C_LECTURER:
			...
			case CS_C_GRADSTUD:
				instances_[i].num = _getRandomFromRange(R_GRADSTUD_FACULTY_MIN
						* instances_[CS_C_FACULTY].total,
						R_GRADSTUD_FACULTY_MAX * instances_[CS_C_FACULTY].total);
				break;
			case CS_C_TA:
				instances_[i].num = _getRandomFromRange(
						instances_[CS_C_GRADSTUD].total / R_GRADSTUD_TA_MAX,
						instances_[CS_C_GRADSTUD].total / R_GRADSTUD_TA_MIN);
				break;
			case CS_C_RA:
				instances_[i].num = _getRandomFromRange(
						instances_[CS_C_GRADSTUD].total / R_GRADSTUD_RA_MAX,
						instances_[CS_C_GRADSTUD].total / R_GRADSTUD_RA_MIN);
				break;
			case CS_C_RESEARCHGROUP:
				instances_[i].num = _getRandomFromRange(RESEARCHGROUP_MIN,
						RESEARCHGROUP_MAX);
				break;
			default:
	...
	private void _generate() {
		...
		try {
			log_ = new PrintStream(new FileOutputStream(
	...
	private void _generateUniv(int index) {
		// this transformation guarantees no different pairs of (index,
		// baseSeed) generate the same data
	...
	private void _generateDept(int univIndex, int index) {
		String fileName = System.getProperty("user.dir") + "\\"
				+ _getName(CS_C_UNIV, univIndex) + INDEX_DELIMITER + index
				+ _getFileSuffix();
	...
	private void _generateAUniv(int index) {
		...
		writer_.addProperty(CS_P_NAME, _getRelativeName(CS_C_UNIV, index),
				false);
	...
	private void _generateADept(int index) {
		...
		writer_.addProperty(CS_P_NAME, _getRelativeName(CS_C_DEPT, index),
				false);
	...
	private void _generateAFaculty_a(int type, int index) {
		...
		for (int i = 0; i < courseNum; i++) {
			...
			writer_.addProperty(CS_P_TEACHEROF,
					_getId(CS_C_COURSE, courseIndex), true);
		}
		// gradutate courses
		courseNum = _getRandomFromRange(FACULTY_GRADCOURSE_MIN,
				FACULTY_GRADCOURSE_MAX);
		for (int i = 0; i < courseNum; i++) {
			courseIndex = _AssignGraduateCourse(indexInFaculty);
			writer_.addProperty(CS_P_TEACHEROF,
					_getId(CS_C_GRADCOURSE, courseIndex), true);
		}
		// person properties
	...
	private int _AssignCourse(int indexInFaculty) {
		// NOTE: this line, although overriden by the next one, is deliberately
		// kept
		...
		course.globalIndex = ((Integer) remainingUnderCourses_.get(pos))
				.intValue();
	...
	private int _AssignGraduateCourse(int indexInFaculty) {
		...
		CourseInfo course = new CourseInfo();
		course.indexInFaculty = indexInFaculty;
		course.globalIndex = ((Integer) remainingGradCourses_.get(pos))
				.intValue();
	...
	private void _generateAProf_a(int type, int index) {
		...
		writer_.addProperty(CS_P_RESEARCHINTEREST,
				_getRelativeName(CS_C_RESEARCH, random_.nextInt(RESEARCH_NUM)),
				false);
	}
	 ...
	 *            Maximum number of publications.
	 */
	private void _assignGraduateStudentPublications(String author, int min,
			int max) {
		int num;
		PublicationInfo publication;
		...
		for (int i = 0; i < list.size(); i++) {
			publication = (PublicationInfo) publications_.get(((Integer) list
					.get(i)).intValue());
			publication.authors.add(author);
		}
	}
	...
	private void _generateAnUndergraduateStudent(int index) {
		...
		for (int i = 0; i < list.size(); i++) {
			CourseInfo info = (CourseInfo) underCourses_.get(((Integer) list
					.get(i)).intValue());
			writer_.addProperty(CS_P_TAKECOURSE,
					_getId(CS_C_COURSE, info.globalIndex), true);
		}
	...
	private void _generateAGradudateStudent(int index) {
		...
		for (int i = 0; i < list.size(); i++) {
			CourseInfo info = (CourseInfo) gradCourses_.get(((Integer) list
					.get(i)).intValue());
			writer_.addProperty(CS_P_TAKECOURSE,
		...
		_assignGraduateStudentPublications(id, GRADSTUD_PUB_MIN,
				GRADSTUD_PUB_MAX);
	...
	private void _generateATa(TaInfo ta) {
		writer_.startAboutSection(CS_C_TA,
				_getId(CS_C_GRADSTUD, ta.indexInGradStud));
		writer_.addProperty(CS_P_TAOF, _getId(CS_C_COURSE, ta.indexInCourse),
				true);
	...
	private void _generateAnRa(RaInfo ra) {
		writer_.startAboutSection(CS_C_RA,
				_getId(CS_C_GRADSTUD, ra.indexInGradStud));
	...
	private void _generateACourse(int index) {
		...
		writer_.addProperty(CS_P_NAME, _getRelativeName(CS_C_COURSE, index),
				false);
	...
	private void _generateRaTa() {
		...
		tas = new ArrayList();
		ras = new ArrayList();
		list = _getRandomList(instances_[CS_C_TA].total
				+ instances_[CS_C_RA].total, 0,
				instances_[CS_C_GRADSTUD].total - 1);
		...
		for (i = 0; i < instances_[CS_C_TA].total; i++) {
			...
			ta.indexInCourse = ((CourseInfo) underCourses_
					.get(((Integer) courseList.get(i)).intValue())).globalIndex;
			_generateATa(ta);
		}
		while (i < list.size()) {
			ra = new RaInfo();
	...
	private String _getId(int classType, int index) {
		...
		case CS_C_DEPT:
			id = "http://www."
					...
					+ "."
					+ _getRelativeName(CS_C_UNIV,
							instances_[CS_C_UNIV].count - 1) + ".edu";
			break;
		default:
			id = _getId(CS_C_DEPT, instances_[CS_C_DEPT].count - 1)
					+ ID_DELIMITER + _getRelativeName(classType, index);
			break;
		}
	 ...
	 * includes the increase of the instacne count of all its super class.
	 * 
	 * @param classType
	 *            Type of the instance.
	 */
	...
	private void _generateComments() {
		...
		System.out.println("CLASS INSTANCE #: " + classInstNum
				+ ", TOTAL SO FAR: " + totalClassInstNum);
