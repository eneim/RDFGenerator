...
public class GmlCurve implements Data {
	...
	public static GmlCurve[] join(List<GmlCurve> curves) {
		 ...
		 * 
		 * LinkedList<GmlCurve> list = new LinkedList<GmlCurve>(curves); while
		 * (!list.isEmpty()) { GmlCurve curve = list.poll();
		 * 
		 * Iterator<GmlCurve> itr = list.iterator(); while (itr.hasNext()) {
		 * GmlCurve c = itr.next(); GmlCurve joined = curve.join(c); if (joined
		 * != null) { itr.remove(); curve = joined; itr = list.iterator(); } }
		 * 
		 * ret.add(curve); } return ret.toArray(new GmlCurve[ret.size()]);
		 */
	}
	...
	private void initBounds() {
		...
		for (int i = 0; i < this.npoints; i++) {
			if (minX > this.xpoints[i])
				minX = this.xpoints[i];
			if (maxX < this.xpoints[i])
				maxX = this.xpoints[i];
			if (minY > this.ypoints[i])
				minY = this.ypoints[i];
			if (maxY < this.ypoints[i])
				maxY = this.ypoints[i];
		}
	...
	public Point getLastPoint() {
		return new Point(this.xpoints[this.npoints - 1],
				this.ypoints[this.npoints - 1]);
	}
	...
	public boolean equals(Object obj) {
		...
		if (obj instanceof GmlCurve) {
			...
			ret = (Arrays.equals(this.xpoints, curve.xpoints) && Arrays.equals(
					this.ypoints, curve.ypoints))
					|| (Arrays
							.equals(this.xpoints, reverseArray(curve.xpoints)) && Arrays
							.equals(this.ypoints, reverseArray(curve.ypoints)));
		}
		return ret;
	}
	...
	@Override
	public int hashCode() {
		return this.xpoints[0] + this.ypoints[0]
				+ this.xpoints[this.npoints - 1]
				+ this.ypoints[this.npoints - 1];
	}
	...
	protected boolean isConnected(GmlCurve curve) {
		...
		return (p1.equals(p3) || p1.equals(p4) || p2.equals(p3) || p2
				.equals(p4));
	}
	...
	public GmlCurve join(GmlCurve curve) {
		...
		if (p1.equals(p3)) {
			...
			for (int i = 1; i < curve.npoints; i++) {
				xary[this.npoints + i - 1] = curve.xpoints[curve.npoints - 1
						- i];
				yary[this.npoints + i - 1] = curve.ypoints[curve.npoints - 1
						- i];
			}
			ret = new GmlCurve(xary, yary, n);
