...
import im.ene.lab.rdf.bench.ksj.map.ksj.NPoint;
import im.ene.lab.rdf.bench.ksj.map.ksj.RailroadSectionData;
import im.ene.lab.rdf.bench.ksj.map.ksj.Station;
import im.ene.lab.rdf.bench.ksj.map.ksj.shelter.HazardClassification;
...
public class KsjHandler extends DefaultHandler {
	...
	public KsjHandler() {
		...
		// shelter
		this.charactersTarget.add("ksj:name");
		this.charactersTarget.add("ksj:address");
		this.charactersTarget.add("ksj:facilityType");
		this.charactersTarget.add("ksj:seatingCapacity");
		this.charactersTarget.add("ksj:facilityScale");
		this.charactersTarget.add("ksj:earthquakeHazard");
		this.charactersTarget.add("ksj:tsunamiHazard");
		this.charactersTarget.add("ksj:windAndFloodDamage");
		this.charactersTarget.add("ksj:volcanicHazard");
	...
	public void fixCharacters() {
		...
		if (this.buf.length() > 0) {
			...
			if ("gml:posList".equals(tag)) {
				...
				data.link(
						tag,
						new NPoint(Double.valueOf(param[0]), Double
								.valueOf(param[1])));
			} else if (data != null) {
